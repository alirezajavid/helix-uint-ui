    ...
    <div class="md-layout md-gutter md-alignment-center">
      ...
      >
        <div class="player">
          ...
          <video width="100%" :src="src" controls></video>
        </div>
      </div>
      <div
        class="md-layout-item md-medium-size-50 md-small-size-100 md-xsmall-size-100"
      >
        <md-table md-card>
          ...
          <md-table-row v-for="footage in footages" :key="footage.name">
            ...
            <md-table-cell>
              <span
                @click="show(footage.href, footage.name)"
    ...
    <md-dialog :md-active.sync="showDialog">
      ...
      <md-dialog-actions>
        <table>
          ...
          <tr>
            <td>
              <md-button class="btn btn-warning mr-1 mb-1 jj" @click="capture">
                <md-progress-spinner
                  class="md-accent"
                  v-show="in_progress"
                  :md-diameter="15"
...
export default {
  ...
  methods: {
    ...
    pooler() {
      axios.get("/api/footage_inquiry?token=" + this.token).then((r) => {
        if (r.data.state == "pending") setTimeout(this.pooler, 5000);
    ...
    human_time(s) {
      ...
      if (s >= 3600)
        return (
          _p(Math.floor(s / 3600)) +
          ":" +
          _p(Math.floor((s % 3600) / 60)) +
          ":" +
          _p(s % 60)
        );
      if (s >= 60)
    ...
    del(footage) {
      axios
        .get(
          "/api/footage_delete?name=" + footage.name + "&len=" + footage.length
        )
        .then((r) => {
          if (r.data.success == true) this.fetch_footages();
        });
    },
    upload(footage) {
      axios
        .get(
          "/api/footage_upload?name=" + footage.name + "&len=" + footage.length
        )
        .then((r) => {
          if (r.data.success == true) window.open(r.data.href, "_blank");
        });
    },
  },
  created() {
